  ...
  this.interval = setInterval(function () {
    ...
    Promise.resolve()
      ...
      .then(function(result) {
        this.log.info('%s overhead %s with %d in %dms', result.state, id, result.count, result.ms);
      }.bind(this))
      .then(function () {
        return this.measureReplication(id, config);
      }.bind(this))
      .then(function(result) {
        this.log.info('%s message %s with %d in %dms', result.state, id, result.count, result.ms);
      }.bind(this))
