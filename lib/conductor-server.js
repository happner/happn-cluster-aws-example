...
ConductorServer.prototype.onConnection = function (data) {
  this.log.info('connect', data.info.name);
...
ConductorServer.prototype.onDisconnection = function (data) {
  ...
  this.members.splice(this.members.indexOf(data.info.name), 1);
};
  ...
  this.interval = setInterval(function () {
    ...
    Promise.resolve()
      ...
      .then(function(result) {
        this.log.info('%s overhead %s in %dms', result.state, id, result.ms);
      }.bind(this))
      .then(function () {
        return this.measureReplication(id, config);
      }.bind(this))
      .then(function(result) {
        this.log.info('%s message %s in %dms', result.state, id, result.ms);
      }.bind(this))
